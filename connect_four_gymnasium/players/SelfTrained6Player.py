...
class ResNet(nn.Module):
  def __init__(self, num_blocks, num_channels):
    ...
    self.start_block = nn.Sequential(
      nn.Conv2d(2, num_channels, kernel_size=3, padding=1), #todo If a nn.Conv2d layer is directly followed by a nn.BatchNorm2d layer, then the bias in the convolution is not needed, instead use nn.Conv2d(..., bias=False, ....). 
